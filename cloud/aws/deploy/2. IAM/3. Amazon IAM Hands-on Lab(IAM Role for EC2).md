...
## 3. Gán Role Cho EC2 và Kiểm Tra Kết Quả (Verify)

### 3.1. Gán IAM Role vừa tạo vào EC2 Instance

1. Truy cập **EC2 Console** > Chọn **Instances** từ menu bên trái.
2. Tích chọn EC2 Instance của bạn > Chọn menu **Actions** ở góc trên bên phải > Chọn **Security** > Chọn **Modify IAM role**.
3. Tại ô tìm kiếm IAM Role, chọn `ec2-role` vừa tạo > Nhấp **Update IAM role** (hoặc **Save**) để lưu cấu hình.

### 3.2. Kiểm tra thao tác với S3 từ bên trong máy chủ EC2 (SSH)

1. Quay lại cửa sổ dòng lệnh đã SSH vào máy chủ EC2 Linux ở Bước 1.
2. Chạy lệnh liệt kê danh sách S3 bucket hiện tại:
   ```bash
   aws s3 ls
   ```
3. **Kết quả mong đợi:** Lệnh chạy thành công và hiển thị danh sách các bucket hiện tại trên tài khoản AWS của bạn mà **không cần** chạy lệnh `aws configure` để cấu hình Access Key ID / Secret Access Key tĩnh. Điều này giúp nâng cao bảo mật vì thông tin xác thực được quản lý tự động và xoay vòng thường xuyên bởi AWS STS.
