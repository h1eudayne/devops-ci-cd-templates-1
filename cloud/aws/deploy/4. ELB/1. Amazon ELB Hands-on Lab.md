...
### Bước 4: Kiểm tra hoạt động định tuyến

1. Để client có thể truy cập cổng `8080` của Load Balancer, bạn cần đảm bảo **Security Group** của Load Balancer đã mở cổng `8080` từ nguồn `0.0.0.0/0`.
2. Sao chép địa chỉ **DNS name** của Load Balancer `alb-01` (ví dụ: `alb-01-1234479115.us-east-1.elb.amazonaws.com`).
3. Truy cập vào trình duyệt theo 2 đường dẫn cổng để kiểm chứng:
   - **Đường dẫn 1 (Cổng 80 mặc định)**:
     ```text
     http://<DNS_NAME_CUA_ALB>
     ```
     -> Trình duyệt luôn luôn trả về giao diện màu xanh dương của **Instance A**: `Welcome to instance - A` (không còn xoay vòng traffic sang B nữa vì B đã bị gỡ khỏi `tg-01`).
   - **Đường dẫn 2 (Cổng 8080)**:
     ```text
     http://<DNS_NAME_CUA_ALB>:8080
     ```
     -> Trình duyệt luôn luôn trả về giao diện màu xanh lá của **Instance B**: `Welcome to instance - B` (do cổng 8080 được định tuyến trực tiếp tới `tg-02` chứa máy chủ B).

Chúc mừng bạn đã hoàn thành cấu hình nâng cao định tuyến lưu lượng qua nhiều Target Group khác nhau thành công!
