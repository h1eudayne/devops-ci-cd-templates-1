...
## III. Khả năng tích hợp với các dịch vụ AWS (Integrations)
...
### 1. Dịch vụ đầu vào / Nguồn kích hoạt (Event Sources / Triggers)
Hàm Lambda được tự động kích hoạt phản hồi lại các sự kiện từ:
* **Tính toán & Mạng**: **Amazon API Gateway** (xử lý request HTTP/HTTPS), **Application Load Balancer (ALB)**.
* **Lưu trữ**: **Amazon S3** (khi có file upload/delete), **Amazon Elastic File System (EFS)**.
* **Cơ sở dữ liệu**: **Amazon DynamoDB** (theo dõi thay đổi qua DynamoDB Streams).
...
* **Streaming dữ liệu**: **Amazon Kinesis** (phân tích data streaming thời gian thực), **Amazon MSK** (Kafka).
* **Thiết bị thông minh**: **AWS IoT**.

### 2. Dịch vụ đầu ra / Đích đến (Destinations / Targets)
Sau khi xử lý xong logic, Lambda có thể đẩy kết quả hoặc tương tác với:
* **Ghi nhật ký**: **Amazon CloudWatch** (tự động ghi nhận Logs và Metrics).
...
* **Lưu trữ**: Đẩy file output hoặc lưu trữ kết quả xử lý vào **Amazon S3**.
* **Dịch vụ thông báo**: Gửi email qua **Amazon SES**, hoặc đẩy tin nhắn qua **Amazon SNS / SQS**.

---

## IV. Mô hình tính giá AWS Lambda (Lambda Pricing)
